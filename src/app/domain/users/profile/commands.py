...
from app.domain.users.core.schemas import UserPartialUpdateFull, UserDetailsFull
from app.domain.users.profile.schemas import UserProfilePartialUpdate, UserPartialUpdate
...
class UserAvatarUpdateCommand:
    ...
    async def __call__(self, payload: UserPartialUpdate) -> None:
        current_user = await self.current_user_query()
        await self.user_partial_update_command(
            current_user.id, UserPartialUpdateFull(**payload.model_dump(exclude_unset=True))
        )
